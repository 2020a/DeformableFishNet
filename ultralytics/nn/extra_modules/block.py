...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_ContextGuided', 'C2f_ContextGuided', 'C3_MSBlock', 'C2f_MSBlock', 'ContextGuidedBlock_Down', 'C2f_DCNv2_DCAS']
...
# 改进使用k=3的空洞卷积
# 变为进行一个1x1的卷积、一个3x3的卷积、一个3x3 d=2的空洞卷积、一个3x3 d=3的空洞卷积
class EMSConvP(nn.Module):
    # Efficient Multi-Scale Conv Plus
    def __init__(self, channel=256, kernels=[1, 3, 3, 3]):
        super().__init__()
        ...
        for index, ks in enumerate(kernels):
            if index == 0:
                self.convs.append(Conv(c1=min_ch, c2=min_ch, k=ks))
            else:
                self.convs.append(Conv(c1=min_ch, c2=min_ch, k=ks, d=index))
...
class DCNv2_Offset_Attention(nn.Module):
    def __init__(self, in_channels, kernel_size, stride, deformable_groups=1) -> None:
        ...
        self.attention = EGCA(self.out_channel)
...
######################################## C3 C2f DCNV2_DCAS start ########################################

class DCNv2_Offset_Attention_DCAS(nn.Module):
    def __init__(self, in_channels, kernel_size, stride, deformable_groups=1) -> None:
        super().__init__()
        ...
        self.out_channel = (deformable_groups * 3 * kernel_size * kernel_size)
        self.conv_offset_mask = nn.Conv2d(in_channels, self.out_channel, kernel_size, stride, padding, bias=True)
        self.attention = DCAS(self.out_channel)
...
class DCNv2_DCAS(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=None, groups=1, dilation=1, act=True, deformable_groups=1):
        super(DCNv2_DCAS, self).__init__()
        ...
        self.conv_offset_mask = DCNv2_Offset_Attention_DCAS(in_channels, kernel_size, stride, deformable_groups)
        self.bn = nn.BatchNorm2d(out_channels)
        self.act = Conv.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.reset_parameters()
...
class Bottleneck_DCNV2_DCAS(Bottleneck):
    """Standard bottleneck with DCNV2."""

    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv2 = DCNv2_DCAS(c_, c2, k[1], 1)


class C3_DCNv2_DCAS(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_DCNV2_DCAS(c_, c_, shortcut, g, k=(1, 3), e=1.0) for _ in range(n)))


class C2f_DCNv2_DCAS(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_DCNV2_DCAS(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))

######################################## C3 C2f DCNV2_DCAS end ########################################
